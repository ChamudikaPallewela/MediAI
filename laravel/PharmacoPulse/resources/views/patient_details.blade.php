...
<a href="{{ url('/redirects') }}" class="brand" style="display: flex; align-items: center; justify-content: flex-start;">
	...
	<section id="content">
        ...
        <div class="mb-4">
            <h4>Patient Name: {{ $patient->patient_name }}</h4>
    ...
    <div class="card-body">
        ...
        <img src="{{ url('storage/qrcodes/' . $patient->id . '.png') }}" alt="QR Code">


        <form action="{{ route('patients.generateQrCode', $patient->id) }}" method="post">
            @csrf
            <button type="submit" class="btn btn-primary mt-3">Generate QR Code</button>
        </form>
		<!-- <form action="{{ route('patients.generateDynamicQrCode', $patient->id) }}" method="post">
		
    @csrf
    <button type="submit" class="btn btn-primary mt-3">Update QR Code</button>
	 -->
<!-- </form> -->
    </div>
</div>
                ...
                <tbody>
                    @foreach ($prescriptions as $prescription)
                        <tr>
                            ...
                            <td>{{ $prescription->patient->clinic->clinic_name }}</td>
                            <td>{{ $prescription->start_date }}</td>
                            <td>{{ $prescription->end_date }}</td>
                            <td>
                                @php
								
                                    $daysMore = \Carbon\Carbon::now()->diffInDays(\Carbon\Carbon::parse($prescription->end_date)->endOfDay() , false);
                                @endphp
                                
                                {{ $daysMore }}

                                @if ($daysMore < 0)
                                    <!-- Automatically delete the row -->
                                    @php
                                        $prescription->delete();
                                    @endphp
                                @endif
