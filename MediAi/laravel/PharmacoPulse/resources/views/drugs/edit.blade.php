...
/* Add spacing and better alignment */
.form-group {
    margin-bottom: 20px;
}
...
<h2>Edit Medicine</h2>
    <form action="{{ route('drugs.update', $drug->id) }}" method="POST">
        ...
        @method('PUT')
        <div class="mb-3">
            <label for="drug_name" class="form-label">Medicine Name</label>
            <input type="text" class="form-control" id="drug_name" name="drug_name" value="{{ $drug->drug_name }}" required>
        </div>
        <div class="mb-3">
            <label for="description" class="form-label">Description</label>
            <textarea class="form-control" id="description" name="description">{{ $drug->description }}</textarea>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="clinic_id" name="clinic_id" required>
                @foreach ($clinics as $clinic)
                    <option value="{{ $clinic->id }}" {{ $drug->clinics->contains($clinic->id) ? 'selected' : '' }}>
                        {{ $clinic->clinic_name }}
                    </option>
                @endforeach
            </select>
